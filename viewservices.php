...
$mysqli = new mysqli('localhost','root','','carcare') or die(mysqli_error($mysqli));	
	$result =  $mysqli->query("SELECT * FROM appointments")or die ($mysqli->erorr);	
...
<li><div align="center"><a class="active" href="viewservices.php">Services</a>	</div></li>	
...
<div class="modal fade" id="editmodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Update Customer Appointments </h5>
		<form method="post" action="viewservices_update.php"  enctype="multipart/form-data"> 	
    ...
    <br>
	 <input type="text" name="customerid" id="customerid" class="form-control" placeholder="Customer ID" readonly/>
    <br>	  
    ...
    <br>
     <input type="text" name="package" id="package" class="form-control" placeholder="Package"  readonly/>
    <br>
     <input type="text" name="vehicaltype" id="vehicaltype"  class="form-control" placeholder="Vehical Type" readonly/>
		   <br>
     <input type="text" name="Vehicalnumber" id="Vehicalnumber"  class="form-control" placeholder="Vehical Number" readonly/>
		   <br>
     <input type="text" name="date" id="date"  class="form-control" placeholder="date" readonly/>
		   <br>
     <input type="text" name="timeslot" id="timeslot"  class="form-control" placeholder="Time" >
		   <br>
     <input type="text" name="servicestatus" id="servicestatus"  class="form-control" placeholder="Service Status" r>
		   <br>
     <input type="text" name="billstatus" id="billstatus"  class="form-control" placeholder="Bill Status" >
...
$result = $mysqli->query("SELECT *  FROM appointments WHERE date = '$search'") ; 
}	 
?>					 						 
    <table align="center">
<form method="POST" class="form">
<th> <input type="date" name="search" class="form-control" width="100px">	  </th> 
<th><input type="submit" class="btn btn-primary" value="Search Date" name="submit2" class="form-group"></th>
</form>
...
{	
$search = $mysqli->real_escape_string($_POST['search']);	
$result = $mysqli->query("SELECT *  FROM appointments WHERE customerid = '$search'" ) ; 
}	 
?>					 						 
    <table align="center">
<form method="POST" class="form">
<th> <input type="text" name="search" class="form-control" width="100px"  placeholder="Enter Customer ID" >	  </th> 
<th><input type="submit" class="btn btn-primary" value="Search Customer" name="submit3" class="form-group"></th>
</form>
 	 </table>
		
			  <hr>  <h1 align="center" class="jumbotron">Service Appointments</h1>
  ...
  <tbody>
    ...
    <tr>
 	 ...
 	 <td><?php echo $row['email']; ?></td>
		 ...
		 <td><?php echo $row['vehicaltype']; ?></td>
  ...
  <script>
		$(document).ready(function() {
			...
			console.log(data);
				...
				$('#email').val(data[3]);
				$('#package').val(data[4]);
				$('#vehicaltype').val(data[5]);
			    $('#Vehicalnumber').val(data[6]);
				$('#date').val(data[7]);
				$('#timeslot').val(data[8]);
				$('#servicestatus').val(data[9]);
				$('#billstatus').val(data[10]);
