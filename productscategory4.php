...
$mysqli = new mysqli('localhost','root','','carcare') or die(mysqli_error($mysqli));	
	$result =  $mysqli->query("SELECT * FROM lightings")or die ($mysqli->erorr);	
	...
	<div id="model" class="modal fade"  tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  		<div class="modal-dialog" role="document">
        		...
        		<h4 class="modal-title" id="exampleModalLabel1">Add Items</h4>
				 ...
				 <hr>
	<form method="post" action="productscategory4_manage.php"  enctype="multipart/form-data"> 
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
		...
		<form method="post" action="productscategory4_update.php"  enctype="multipart/form-data"> 	
...
$result = $mysqli->query("SELECT *  FROM lightings WHERE category = '$search'") ; 
}	 
?>					 						 
    <table align="center">
...
<h1 class="jumbotron" align="center">Lightings</h1>
<table class="table table-striped table-hover" id="user-data">
  ...
  <tbody>
	...
	<td><button type="button" class="btn btn-info btn-xs editbtn"><i class="fa fa-edit" aria-hidden="true"></i></button></td>
 	<td><a href="productscategory4_manage.php?delete= <?php echo $row['id']; ?> " class="btn btn-danger btn-xs"><i class="fa fa-trash"></a></td>
